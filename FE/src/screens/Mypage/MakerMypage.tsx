...
export default function MakerMypage({ userRole, onLogout }: MakerMypageProps) {
    ...
    <View style={styles.container}>
      ...
      {selectedItem ? (
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          {/* 프로필 섹션 */}
          <ProfileSection userType="maker" userName="Maker" />
        ...
        <View style={styles.tabContent}>
              ...
              <View style={styles.gridContainer}>
                {storeReviewsData.map((item, index) => (
                  <MypageGridComponent
                    ...
                    onPress={() => {
                      // TODO: 리뷰 상세보기로 이동
                      console.log("가게 리뷰 상세보기:", item.title); 
                    }}
                  />
                ))}
              </View>
          ...
          {/* 스토리 이벤트가 없을 때입니다.. 일단 일일히 넣을 수 있는데 없을 때는 이렇게 넣고, 분기처리 고민... */}
          {activeTab === "storeEvents" && (
            <View style={styles.emptyContent}>
              <Text style={styles.emptyText}>가게 이벤트가 없습니다</Text>
            </View>
          )}

          {activeTab === "receivedMenuBoard" && (
            <View style={styles.emptyContent}>
              <Text style={styles.emptyText}>받은 메뉴판이 없습니다</Text>
            </View>
          )}
        </View>
      </ScrollView>
