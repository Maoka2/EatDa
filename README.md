...
1. **데이터 로더 병목 & RAM 폭증**
    - 원인: 모든 경로를 한 번에 리스트업·검증, `random.sample(50000)` 등으로 전체 메모리 사용.
    - 결과: I/O 병목, RAM 폭증.
    - 해결: Manifest 기반 스트리밍, IterableDataset + persistent workers 적용.
2. **체크포인트·스케일링 불일치 → 품질 저하/NaN/검은 출력**
    - 원인: LoRA rank/alpha 값 과도 → 디테일 붕괴.
    - 해결: **안정 조합** → rank=8, alpha=16, lora_dropout=0.05.
    - 추가: 텍스트 인코더 동결 + 마지막 10~20% 스텝에서 낮은 LR로 부분 해제.
3. **VRAM 한계**
    - 원인: 모델 본체 FP32로 로드 + LoRA 얹을 때 피크 VRAM 증가.
    - 해결 방법:
        - 텍스트 인코더 동결(default) → **언어 의미 공간 과적합 방지 + VRAM 절감**.
        - LoRA는 필요 시점에만 attach (merge 대신 로드) → **수치 오차 최소화 + 안정성**.
        - 훈련이나 추론할 때만 불러와 쓰면, 메모리 부담도 줄고 수치적으로 안정적
        - 글을 숫자로 바꾸는 임베딩은 CPU on-the-fly 계산 → **GPU는 U-Net 학습에 집중, 병렬 오버랩 효과**.

### 결과

- SD3.5 + LoRA 학습에서 **VRAM 한계와 데이터로더 병목**을 해결 → 학습 파이프라인을 **안정 상태**로 고정.
- 이후 구조를 더 건드리기보다는, 남은 품질 문제(문장 해석 편차, 한글 가독성, 스타일 일관성)는 **프롬프트 품질**에 달려있음을 확인.
...
- 사용자 프롬프트 입력 → 모델 특정 프롬프트 가이드라인에 따른 재작성 요청 → 프롬프트 재생성
    
    → 프롬프트가 일정하지 않다.
    
    → LLM에 너무 의존적이고, 할루시네이션과 맥락에 맞지 않은 텍스트 출력이 너무 많음
    

### 2차 파이프 라인

> 기본적인 음식 생성에 대한 가이드라인을 제공해 맥락성을 높이고 음식 리뷰에 특화된 프롬프트 작성이 필요했음
> 
- 모델 특정 프롬프트 가이드라인에 Few shot 데이터 삽입
    - LLM을 통해 50개 생성 후, 직접 답변을 검증하고 가이드라인에 맞게 재수정함
    
    → Few shot을 통해 좋은 답변이 나오기 시작함
    
    → 그러나, 비슷한 질문에 대한 과적합이 생기고, 동일한 형태의 답변 및 ai 리뷰가 생성이 되었음
    

### 3차 파이프라인

> 과적합 문제를 해결하기 위해서 **루브릭 점수**: 체크리스트 기반 품질 평가를 진행
> 
- 생성된 프롬프트에 대해 5가지 항목(주제/행동/장면/스타일/카메라/조명)에 대해 점수 체크를 진행
- 점수가 낮으면 재생성하도록 루프를 돌림(최대 3회)
- few shot 데이터 숫자를 줄이고, 맥락만 참고하도록 프롬프트를 수정
    
    → 점수화를 통해서 일관된 답변이 줄어들고, 다양한 답변이 생성되기 시작
    
    → 그러나, 점수를 산정하는 방식이 너무 지극히 주관적이고, 객관적인 지표가 필요했음
    
    → 재생성을 해도 품질이 좋아지지 않는 문제점이 발생
    

### 4차 파이프라인

> 객관적 지표 생성을 위한 LLM 도입 + 재생성 플로우 분기
> 
- checklist key를 통한 점수 산정 + LLM self refine 기반 점수 산정 / 2 = 최종 점수
- LLM을 도입해 추가적으로 주관적으로 점수 산정되는 플로우 해결
- 재생성은 5가지 항목 중 부족한 부분 재생성 / 전체 재생성 2가지 분기
    - 더 점수가 높은 프롬프트 채택 되도록 시행
- 루프도 최대 3회로 제한
    
    → 주관적 지표에 대해 완벽히 해결하지 못함
    
    → 루프를 돌다보면 시간이 오래 걸린다는 문제점 직면
    

### 5차 파이프라인

> 루프를 통해 생성된 프롬프트 및 추가 텍스트 기록
> 
- 재생성 루프를 통해 점수가 높아진 프롬프트에서, 점수를 높이게 된 텍스트를 LLM이 추출
- 점수가 높아진 프롬프트의 전/후를 데이터화
    
    → 추후 SFT 등 미세튜닝에 데이터 사용 가능성 (서비스 확장성 고려)
    
    → 미세 튜닝을 통한 프롬프트 작성 시간 단축 가능
</details>